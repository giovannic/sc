...
export async function runTests(): Promise<void> {
  ...
  try {
    ...
    if (created.contextId === testId) {
      ...
      console.log(
        "✗ createContext returned wrong ID"
      );
    }
    ...
    // Test updateReadme
    const updated = await client.updateReadme(
      testId,
      "Updated"
    );
    if (updated.success) {
  ...
  (global.fetch as any) = createMockFetch(
    mockFetchResponses
  );
  const errorClient = new SCClient(
    "http://localhost:3000"
  );
